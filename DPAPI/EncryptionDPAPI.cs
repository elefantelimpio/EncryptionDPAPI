...
namespace DPAPI
{
    public sealed class EncryptionDPAPI
    {
        ...
        // Flag indicating the type of key. DPAPI terminology refers to key types as user store or machine store.
        public  enum KeyType {
            UserKey = 1,
            MachineKey
        };
        ...
        {
            return EncryptValue ( defaultKeyType, plainText, string.Empty, string.Empty );
        }
        ...
        /// </summary>
        /// <param name="keyType"> Defines type of encryption key to use. When user key is specified, any application running under the same user account as the one making this call, will be able to decrypt data.
        /// Machine key will allow any application running on the same computer where data were encrypted to perform decryption. 
        /// Note: If optional entropy is specifed, it will be required
        /// for decryption.
        /// </param> 
        /// <param name="plainText"> Plaintext data to be encrypted. </param>
        /// <returns> Encrypted value in a base64-encoded format. </returns>
        public static string EncryptValue (KeyType keyType, string plainText)
        {
            return EncryptValue ( keyType, plainText, string.Empty, string.Empty );
        }
        ...
        /// <returns> Encrypted value in a base64-encoded format. </returns>
        public static string EncryptValue (KeyType keyType, string plainText, string entropy)
        {
            return EncryptValue ( keyType, plainText, entropy, String.Empty );
        }
        ...
        /// </summary>
        /// <param name="keyType">
        /// Defines type of encryption key to use. When user key is specified, any application running under the same user account  as the one making this call, will be able to decrypt data. 
        /// Machine key will allow any application running on the same computer where data were encrypted to perform decryption. 
        /// Note: If optional entropy is specifed, it will be required for decryption.
        /// </param>
        /// <param name="plainText">  Plaintext data to be encrypted. </param>
        /// <param name="entropy"> Optional entropy which - if specified - will be required to perform decryption. </param>
        /// <param name="description"> Optional description of data to be encrypted. If this value is specified, it will be stored along with encrypted data and returned as a separate value during decryption.</param>
        /// <returns> Encrypted value in a base64-encoded format. </returns>
        public static string EncryptValue (KeyType keyType, string plainText, string entropy, string description)
        {
        ...
        public static byte [] EncryptValue (KeyType keyType, byte [] plainTextBytes, byte [] entropyBytes, string description)
        {
            // Make sure that parameters are valid.
            ...
            DPAPI.DATA_BLOB plainTextBlob = new DPAPI.DATA_BLOB ();
            DPAPI.DATA_BLOB cipherTextBlob = new DPAPI.DATA_BLOB ();
            DPAPI.DATA_BLOB entropyBlob = new DPAPI.DATA_BLOB ();

            // We only need prompt structure because it is a required
            // parameter.
            DPAPI.CRYPTPROTECT_PROMPTSTRUCT prompt = new DPAPI.CRYPTPROTECT_PROMPTSTRUCT ();
            ...
            try
            {
                ...
                bool success = DPAPI.CryptProtectData ( ref plainTextBlob, description, ref entropyBlob, IntPtr.Zero, ref prompt, flags, ref cipherTextBlob );
        ...
        /// <remarks> When decrypting data, it is not necessary to specify which 
        ...
        public static string DecryptValue (string cipherText)
        {
            ...
            return DecryptValue ( cipherText, String.Empty, out description );
        }
        ...
        /// the signature of encrypted data.
        /// </remarks>
        public static string DecryptValue (string cipherText, out string description) 
        {
            return DecryptValue ( cipherText, String.Empty, out description );
        }
        ...
        /// the signature of encrypted data.
        /// </remarks>
        public static string DecryptValue (string cipherText, string entropy,  out string description)
        {
            // Make sure that parameters are valid.
            if ( entropy == null )
            {
            ...
            return Encoding.UTF8.GetString (
                        DecryptValue ( Convert.FromBase64String ( cipherText ),
                                    Encoding.UTF8.GetBytes ( entropy ),
                                out description ) );
        }
        ...
        /// the signature of encrypted data.
        /// </remarks>
        public static byte [] DecryptValue (byte [] cipherTextBytes, byte [] entropyBytes, out string description)
        {
            // Create BLOBs to hold data.
            DPAPI.DATA_BLOB plainTextBlob = new DPAPI.DATA_BLOB ();
            DPAPI.DATA_BLOB cipherTextBlob = new DPAPI.DATA_BLOB ();
            ...
            try
            {
                ...
                // Disable any types of UI. CryptUnprotectData does not mention CRYPTPROTECT_LOCAL_MACHINE flag in the list of supported flags so we will not set it up.
                int flags = CRYPTPROTECT_UI_FORBIDDEN;

                // Call DPAPI to decrypt data.
                bool success = DPAPI.CryptUnprotectData ( ref cipherTextBlob, ref description,  ref entropyBlob, IntPtr.Zero, ref prompt, flags, ref plainTextBlob );
